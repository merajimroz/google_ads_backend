import uuid
from adflare.api.asset.image_asset import AdflareAdImageAsset
from adflare.api.asset.video_asset import AdflareVideoAsset
...
_GEO_LOCATION_1 = "Hyderabad"
_GEO_LOCATION_2 = "New Delhi"
_GEO_LOCATION_3 = "Ahmedabad"
...
def responsive_display_ad(client, customer_id,  ad_details, ad_group_resource_name, dir):
 
    ad_image_asset  = AdflareAdImageAsset(client, customer_id)

    images_names = [ad_details.get('image_1'), ad_details.get('image_2')]
    images_resource_names = []
    for asset_name in images_names:
        url = f'/var/www/adflaredev/backend/google_ads_backend/assets/{dir}/{asset_name}'

        resource_name = ad_image_asset.add_images(url, asset_name)
        if resource_name:
            images_resource_names.append(resource_name)

    url = f"/var/www/adflaredev/backend/google_ads_backend/assets/{dir}/{ad_details.get('image_2')}"
    square_image_resource_names = [ad_image_asset.square_marketing_images(url, ad_details.get('image_2'))]

    ad_video_asset = AdflareVideoAsset(client, customer_id)
    video_id = ad_details.get('video_id')
    video_title = ad_details.get('video_title')
    video_resource_name = ad_video_asset.upload_videos(video_id, video_title)
        
    create_ad_group_ad(
        client, customer_id, ad_group_resource_name, images_resource_names, square_image_resource_names, [video_resource_name], ad_details
    )
...
def create_campaign_budget(client, customer_id):
    ...
    # campaign_budget = campaign_budget_operation.create
    # campaign_budget.name = f"Campaign budget {uuid.uuid4()}"
    # campaign_budget.type = client.enums.BudgetTypeEnum.FIXED_CPA
...
def create_ad_group(client, customer_id, campaign_resource_name):
    ...
    ad_group.name = f" Adgroup Testing RSA via API {uuid.uuid4()}"
    print(ad_group.name)
...
def create_ad_group_ad(
    client, customer_id, ad_group_resource_name, images_resource_names, square_marketing_images_resource_name, video_resources_name, ad_details
):
    """Creates ad group ad.
    ...
    print(ad_details)
    ad_group_ad.ad.final_urls.append(ad_details.get('url'))

    # business name
    ad_group_ad.ad.responsive_display_ad.business_name = ad_details.get('business_name')
    ad_group_ad.ad.responsive_display_ad.marketing_images = images_resource_names
    ad_group_ad.ad.responsive_display_ad.square_marketing_images =square_marketing_images_resource_name

    # long headline
    long_headline = ad_details.get('long_headline')
    ad_group_ad.ad.responsive_display_ad.long_headline = create_ad_text_asset(client, long_headline)
    # Headline 2 and 3

    headline_1 = ad_details.get('headline1')
    headline_2 = ad_details.get('headline2')

    ad_group_ad.ad.responsive_display_ad.headlines.extend(
        [
            create_ad_text_asset(client, headline_1),
            create_ad_text_asset(client, headline_2),
        ]
    )

    # Description 1 and 2
    description_1 = ad_details.get('description1')
    description_2 = ad_details.get('description2')

    ad_description_1 = create_ad_text_asset(client, description_1)
    ad_description_2 = create_ad_text_asset(client, description_2)

    ad_group_ad.ad.responsive_display_ad.descriptions.extend(
        [ad_description_1, ad_description_2]
    )

    # adding videos to ad
    ad_group_ad.ad.responsive_display_ad.youtube_videos = video_resources_name
